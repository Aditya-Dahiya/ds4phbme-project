...
#Step 1 = create evaluation metrics function
####################################################### FOR VINAYAK ######################################################
...
# 
# 
# p_train = predict(lr, newdata=train)
# eval_metrics(lr, train, p_train, target = 'value')
# 
# 
# p_test = predict(lr, newdata=test)
# eval_metrics(lr, test, p_test, target = 'value')
##################################################### FOR VINAYAK #######################################################
...
renderText({
    coeffs[1]*input$CMAQ + coeffs[2]*input$edu + coeffs[3]*input$aod + coeffs[4]*input$area + coeffs[5]*input$emission
...
```{r}
######### This is what i was trying: to put all the predictors into a data frame and then getting a single predicted value. But going wrong in reactivity




# 
#    dataset = reactive(as.data.frame(c(input$CMAQ,input$edu,input$aod,input$area,input$emission)))
#    pred = predict(lr, newdata = dataset())
# 
# 
# renderPrint({pred})
